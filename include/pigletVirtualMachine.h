...
interpretResult vmInterpretNoRangeCheck(uint8_t *bytecode);

interpretResult vmInterpretThreaded(uint8_t *bytecode);

uint64_t vmGetResult(void);

interpretResult vmInterpretTrace(uint8_t *bytecode);

uint64_t vmTraceGetResult(void);

interpretResult vmRcacheInterpret(uint8_t *bytecode);

interpretResult vmRcacheInterpretNoRangeCheck(uint8_t *bytecode);

interpretResult vmRcacheInterpretThreaded(uint8_t *bytecode);

uint64_t vmRcacheGetResult(void);

interpretResult vmRcacheInterpretTrace(uint8_t *bytecode);

uint64_t vmRcacheTraceGetResult(void);
